...
export default function Comprehension({menuOpen, setMenuOpen}) {
  ...
  async function generateVocab(url = 'http://localhost:8000/downloadVideo', params = {}) {
    let url_w_query = url + '?' + ( new URLSearchParams( params ) ).toString();
    ...
    <div id="comprehension" className="comprehension">
      ...
      <form onSubmit={handleSubmit} className="form text-retrieval-form">
        {/* <label className="form-label"> */}
          {/* URL: */}
        <input className="form-input" type="text" name="name" id="name" placeholder="Your URL" value="https://www.youtube.com/watch?v=rxOQtLbaeuw&ab_channel=LeMonde"></input>
        {/* </label> */}
        <label>
          Language:
          <input type="text" name="lang" id="lang" placeholder="Language of the text" value="fr"></input>
        </label>
        <label>
          API Key:
